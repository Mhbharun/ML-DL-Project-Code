...
name = []
import os
for file in os.listdir("logs/lstm3layer/"):
  model.load_weights("logs/lstm3layer/"+file)
...
model.load_weights("logs/lstm3layer/" +name[score.index(min(score))])
testPredict = model.predict(test)
...
np.savetxt("res/lstm3layer/lstm3layer_exact.txt", dataoriginal)
np.savetxt("res/lstm3layer/lstm3layer_predict.txt", inv_testPredict)
