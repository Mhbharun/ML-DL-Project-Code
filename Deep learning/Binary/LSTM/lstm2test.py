...
name = []
import os
for file in os.listdir("logs/lstm2layer/"):
  model.load_weights("logs/lstm2layer/"+file)
...
model.load_weights("logs/lstm2layer/" +name[score.index(min(score))])
testPredict = model.predict(test)
...
np.savetxt("res/lstm2layer/lstm2layer_exact.txt", dataoriginal)
np.savetxt("res/lstm2layer/lstm2layer_predict.txt", inv_testPredict)
