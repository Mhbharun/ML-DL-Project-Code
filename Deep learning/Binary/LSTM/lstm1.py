...
# Train
checkpointer = callbacks.ModelCheckpoint(filepath="logs/lstm1layer/checkpoint-{epoch:02d}.hdf5", verbose=1, save_best_only=True, monitor='loss')
csv_logger = CSVLogger('logs/lstm1layer/lstmtrainanalysis1.csv',separator=',', append=False)
model.fit(train, train_label, nb_epoch=1000, batch_size=32, callbacks=[checkpointer,csv_logger])
model.save("logs/lstm11layer/lstm_model.hdf5")
